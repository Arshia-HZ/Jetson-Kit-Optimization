...
class Featex2():
    def __init__(self, model1, model2, use_cuda):
        ...
        self.input_shape_2 = (1, 3, 480, 640) ### >>> this line should be equal to original picture shape (please change this based on input shape) i.e. (1, 3, 225, 384)
    ...
    def __call__(self, target, mode='big'):
        ...
        ###################################################
        if(target.shape==self.input_shape_4):
            self.model1 = self.layer1to2_trt_4
            self.model2 = self.layer1to16_trt_4
        elif(target.shape==self.input_shape_2):
            self.model1 = self.layer1to2_trt_2
            self.model2 = self.layer1to16_trt_2
        elif(target.shape==self.input_shape_3):
            self.model1 = self.layer1to2_trt_3
            self.model2 = self.layer1to16_trt_3
        else:
            self.model1 = self.layer1to2_trt_1
            self.model2 = self.layer1to16_trt_1
...
class CreateModel():
    def __init__(self, alpha, model1, model2, model3, use_cuda):
        self.alpha = alpha
        self.featex = Featex(model=model1, use_cuda=use_cuda)
...
class QATM():
    def __init__(self):
        ...
        self.model = CreateModel(model1=model1, model2=model2, model3=model3 , alpha=25, use_cuda=True)
...
def plot_result_multi(image_raw, predictions):
    d_img = image_raw.copy()
...
while True:
    ...
    start = time.time()
    predictions = qatm.match(target=cam_img, temp_features=featex_list)
    end = time.time()
	
    print("The inference time in secs ", end - start)

    d_img = plot_result_multi(cam_img, predictions)
