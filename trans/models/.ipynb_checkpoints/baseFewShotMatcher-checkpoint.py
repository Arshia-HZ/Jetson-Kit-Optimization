...
class BaseFewShotMatcher():
    ...
    def predict_multiscale(self, target, templates, temp_boxes, patch_shape=512, vers='old'):
        if vers != 'old':
            self.set_templates(templates, temp_boxes)
        ...
        for scale in self.scales:
            ...
            max_score = 0.0
            crop_box = []
            tar_boxes = self.get_slice_bboxes(resized.shape[0], resized.shape[1],patch_shape, patch_shape, overlap_height_ratio = 0.25, overlap_width_ratio=0.25)
            for tar_box in tar_boxes:
                ...
                time_list.append((consumed_time, len(templates)))
        ...
        return predictions[:10], time_list
    ...
    def evaluate(self, args, threshold=0.5, topn_n_acc=10):
        ...
        for csv_path in csv_paths:
            ...
            for i in tqdm(range(len(csv_file))):
                ...
                for shot in range(0, select_shot):                        
                    ...
                    temp = cv2.imread(os.path.join(base_path, path))
                    if temp is not None:
                        Temp = temp.copy()
                    ...
                    if multi_scale:
                        predictions, time_list = self.predict_multiscale(tar, temps, boxes_, patch_shape, args.version) # 512
        ...
        whole_tm = 0
        whole_tm_count = 0
        for tmlist in times_list:
            for consumedTime, len_tem in tmlist:
                whole_tm_count += 1
                whole_tm += consumedTime

                if len_tem == 1:
                    single_tem_count += 1
                    single_tem += consumedTime
                else:
                    batched_tem_count += 1
                    batched_tem += consumedTime
